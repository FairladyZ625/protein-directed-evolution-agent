...
- **`F-885537A3`(两目标两策略的最终定性)—— 仍然成立,不退役。** 它由 30 seed × 7 方法矩阵支撑,本任务没有产生任何与之冲突的观测;相反,本任务恰恰是因为 v1.0 写于该事实成立之前,才需要在 v2.0 按它重写结论章。本任务的完成不改变该事实的效力。
- **`F-CCF29A66`(外部证伪verdict:池内峰 8.4162 低于 cold-start incumbent 9.536457)—— 仍然成立,不退役。** v1.0 的问题设定章未正面自曝这一条,这是 v2.0 必须补上的诚实边界,而不是对该事实的反驳。

两条均无超越性观测,处置为「继续持有」。若 v2.0 的重写过程中出现与之冲突的实测,按 supersede 链另行记录,不在本任务处置。
