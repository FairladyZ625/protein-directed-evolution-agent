...
`harness/reports/agentic-v0.6/` 的实验数据在主线上,**但产生它的代码不在**。
...
10 / 9 / 6 次。`harness/reports/agentic-v0.6/report.md` 引用的
...
| `harness/reports/agentic-v0.6/report.md` | v0.6 声称的结果与复现命令,合完要能跑 |
| `harness/reports/agentic-v0.5/report.md` | v0.5 的结果,合完**必须仍然可复现** |
...
`harness/reports/workflow-v1.1/gb1/campaign_llm.*`,与本任务文件面不重叠。

## Execution Surface

**可写**:`agent/auto_researcher.py`、`tests/test_auto_researcher_*.py`、
`harness/reports/agentic-v0.6/report.md`(只改其中失效的路径引用)。

**禁区**:
- `harness/reports/` 下任何 metrics / events / figures 产物 —— **不重跑、不覆盖**。
  本任务是把代码补回主线,不是重做实验。
- `harness/reports/workflow-v1.1/`(CEO 在写)
...
5. 修 `harness/reports/agentic-v0.6/report.md` 里指向不存在文件的引用
...
- `harness/tasks/<本任务>/artifacts/merge-verification.md`:
...
- CEO 会确认 `harness/reports/` 下没有任何产物被改动(`git diff --stat` 里不应出现)。
