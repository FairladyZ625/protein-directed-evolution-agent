...
|---|---|---|---|---|
| P0-a | **报告 PDF 页数硬性不符试题**（交付 44 页 / README 入口 25 页，对 3—5 页） | H33 | **需 CEO 先裁决裁剪方案**（checklist:137-139 的 a/b/c）；技术上正文八章约 15.8K 字符可压入 5 页，附录 A–L.3 共 30 节约 40K 字符另立单独 PDF | 裁决后 0.5–1 天 |
| P0-b | README 报告入口仍指 v0.6，当前交付版为 v0.7 | H39 + D5 | 改 `README.md:11-12` 与 `README.zh-CN.md:11-12` 指向 v0.7；受控 PDF 已是 v0.7 真源产物，**无需重制 PDF**。**执行依赖(2026-09-14 记)**:P0-b 要改的入口与 P0-a 裁剪后的产物是**同一处**。在裁剪方案未定时单独执行 P0-b,会让入口指向一个即将被替换的 44 页 PDF,并把页数缺口由 25 页(v0.6)**扩大**到 44 页(v0.7)。因此 **P0-b 应与 P0-a 一并执行**,由裁剪产物决定入口终值;本轮据此**未擅自改动 README** | 0.5 小时(随 P0-a 落地) |
