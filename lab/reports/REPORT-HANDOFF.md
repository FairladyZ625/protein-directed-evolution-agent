...
复核:`data/aav/full_data.csv` 的 `mutated_region` 列共 **38,293** 条唯一 28aa 序列(口径:长度为 28 的唯一取值;`evolution/datasets.py` 的 `load_aav` 另加 `^[A-Z]+$` 且无 `*` 过滤后为 **38,265**,差 28 条,实验实际用的是后者),
...
`lab/reports/workflow-v1.1/gb1/figures/predictor_comparison.png`;测试 `tests/test_predictor.py`。
...
### 7.10 本节引用的 fact 索引(写报告时可直接查证)

今天这一轮审计新记的 fact,按报告里会用到的顺序:

| Fact | 说的是什么 | 会影响报告哪里 |
|---|---|---|
| `F-363846B5` | LLM Critic 端口契约违约,此前一次都没成功过 | 任何「LLM 评审」表述 |
| `F-ABD9B83F` | 修好后 LLM Critic 真实成功率 10 次里 5 次,另 5 次 90 秒超时回退;每轮有调用预算上限(无上限则第 2 轮 6508 个候选需约 34 小时) | LLM 参与度的定量描述 |
| `F-6D573C04` | `make campaign` / `make baseline` 在干净 clone 上跑不起来(已修 `f68f8d7`) | 「按 README 即可复现」 |
| `F-35BD26B9` | CI 连续多次 push 全红:手写 pip 清单与 requirements 漂移,三个测试文件从未真正跑过 | 工程质量一节 |
| `F-94CBEC61` | 数据在主线、生产代码不在主线(已修 `b78bac5`) | 可复现性一节 |
| `F-61A84D3B` | sparse regime 下只有知识增强摸到全局最优;easy 下知识库无可测增益 | 知识增强的结论 |
| `F-6394AB50` | 随机基线的单 seed / 多 seed 引用陷阱 | 所有随机基线数字 |
| `F-8626766F` | demo 五面板经 CEO 浏览器逐个点检通过 | demo 一节 |
| `F-10BF9D02` | 一键安装/运行在真正干净环境实测通过,含缺数据降级阴性对照;71 个 manifest sha256 全部对得上 | 交付与复现一节 |
| `F-281F1B9F` | 前端任务计划里「3 个既存失败」的前提是错的,实测只有 1 个,成因是测试断言漂移而非渲染缺陷 | (仅台账,报告不必提) |

**一条给写报告者的元建议**:上面十条里有六条的共同形态是**「门看起来在、实际没关」**
——测试没被门控、依赖没装、fallback 把契约违约伪装成提供方不可用、默认输出路径指向证据
而不是草稿、指标卡默认展示统计上最弱的那个字段。如果报告要写一段工程反思,
这个形态比逐条罗列缺陷更有价值:**每一处都不是「代码写错了」,而是「验证手段本身失效了
却不出声」**。

### 7.11 ⛔ 已作废——本节结论是错的,不要引用。改引 §7.14

> **2026-09-13 撤回。** 本节把「LLM 臂没花完预算」误读成了「LLM 让批次质量下降」。
> llm 档的 agent 策略只提名了 105 / 197 次,而对照的 hard 档提名了 288 次;
> 按每次提名归一化之后,**LLM 臂的命中率反而更高**(0.305 vs 0.174)。
> 本节表格里的 −36% / −52% 是预算差,不是质量差。
> 本节「knowledge_agent 比 agent_no_knowledge 好」的部分同样不成立——
> 那只是它多拿了一个满额确定性回退轮次。
>
> **正确的结论与证据见 §7.14。下面的原文保留仅为留痕,任何一句都不要写进报告。**

### 7.11(已作废,原文留痕)接入真 LLM 之后,两个 agent 策略都变差了

这是修好 LLM Critic 端口(7.1)之后第一次跑出来的、真正有 LLM 参与的四策略结果,
**而且这个对比自带阴性对照**。

`llm` 档与 `hard` 档的冷启动(HD≤2)、预算(96×3)、轮数、候选空间(149,361)、
oracle 完全相同,唯一变量是假设生成与 Critic 两个端口接不接池内 LLM(`gpt-5.6-sol`)。
...
**random 与 greedy 逐位相同 = 天然的阴性对照。** 这两个策略不经过 LLM 端口,
它们没变,证明两次运行之间除了 LLM 没有别的东西在动 ——
所以 agent 两行的下降不能被归因于随机性或环境差异。

三个模型策略的峰值**都仍是 8.761966**:找峰的能力没变,变差的是批次产出。
机制上说得通 —— 找峰靠 Ridge 代理模型对全空间排序(LLM 不参与),
而 LLM 参与的是「从实测单点替换里挑哪些去组合」,它挑出的集合比确定性排序更窄。

**报告怎么用**:这直接回答试题考核重点的最后一问 ——
「讨论 LLM Agent 是否真正学到了『科学家思维』,还是只是调用预测模型」。
**本仓的证据支持后者偏多**:把 LLM 放进假设生成角色,批次质量下降而非上升;
真正把峰找出来的是预测模型。

这是个负结果,但它是本次交付里**最有说服力的一段实验设计**:同条件、单变量、
自带阴性对照。建议在报告里按「我们做了一个受控对照,结论对我们自己不利,但我们如实报告」
来写,而不是淡化处理。试题的考核重点原文就是
「是否能分析模型失败原因,而不是只展示成功样例」。

配套要写明的口径(fact `F-ABD9B83F`):LLM Critic 全程写出实质评审 27 条、
90 秒超时回退 23 条,**成功率 54%**;假设生成端口逐轮来源
agent_no_knowledge = `llm / fallback / llm`,knowledge_agent = `llm / fallback / fallback`。
...
此前事件流里逐变体真值只存在于 `campaign.round.completed` 的 top10 字段,
而失败的提名——尤其「预测很高、实测致死」那种——按定义进不了 top-10,
**所以失败案例结构性地一条都没被记录过**。这也是试题「结果展示 d-iv:哪些推荐失败,
原因可能是什么」此前答不上来的根本原因,不是没写,是没数据。

第一轮单因子对照(AAV,seed 42,288 预算,两臂都记残差、只有一臂注入):

| | control | reflexion |
|---|---|---|
| `redirect_batch` 触发轮次 | `[5]` | **`[2, 5]`** |
| 六轮实测批次 | — | **与 control 逐位相同(48/48 × 6)** |
| cum_top10_max / strong | 8.4162 / 157 | 8.4162 / 157 |
| LLM 成功率(240s 超时) | 6/6 | 6/6 |

**两句话可以引用,但必须一起引:**

1. 注入**确实改变了 agent 的动作**——basin hop 从第 5 轮提前到第 2 轮,
   正是 RFC-008 预测的「提前 1~2 轮」。
2. 但**实测批次一个变体都没变**。那次提前的 hop 是空转的:第 2 轮还没形成
   可供「跳离」的最优簇。净效果为零。

**成因与构型强绑定,这一点务必写清**:两臂每一轮的采集参数都是
`src=v06_pure_exploit_default`、`requested=1.0 → effective=1.0`,
LLM 从头到尾没有对采集比例行使过任何裁量。**本轮测到的不是「反思没用」,
而是「在纯利用构型下反思无处着力」。** 2×2 因子实验(采集构型 × 反思开关)在跑,
结论以那一轮为准。

**最适合写进正文的一条**:reflexion 臂的 LLM 在总结里声称
"Residual reflexion excluded the two strongly lethal round-5 contexts and prioritized
measured-supported motifs",而它实际测的 288 个变体与对照臂逐字节相同——
**模型声称据此做了取舍,做的事却一件没变**。这条直接答试题
「LLM Agent 是否真正学到了『科学家思维』,还是只是调用预测模型」。

**硬约束(违反就是过度声称)**:
- 这是 **n=1**,底下还是一个有随机性的 LLM。本仓已有前车之鉴(fact `F-6394AB50`:
  随机基线单 seed 看似有趋势,5 seed 一算完全重叠)。**不得据此断言反思有效或无效。**
- 「批次逐位相同」这个事实是硬的、可独立复核(取 `agent.tool.test.residuals` 里
  `records[].seq` 的集合哈希);但它的**成因**只在本构型下成立。
- 不要把「redirect 提前 3 轮」单独写成正面结果,必须连着「那次 hop 空转」一起写。
- LLM 成功率 100% 对应的是 **240 秒**超时;此前 GB1 `llm` 档的 54%/50% 对应 **90 秒**
  (fact `F-ABD9B83F`)。**两个数字属于不同实验条件,不能并列比较。**

相关 commit:`a75292b`(合并)、`80f4447`(GB1 侧同类残差记录)。

### 7.14 【更正 §7.11·2026-09-13】LLM 不是选得差,是每次跑通都把提名数打塌到 ~5

**这一节作废并取代 §7.11。** 起因是按用户裁定「真实性最重要」对 llm 档做了一次
独立复现(workflow-v1.2),两次运行差得离谱,查下去发现原结论的比较基准就是错的。

#### 事实

llm 档与 hard 档**并不是同预算比较**。预算名义上是 96 提名/轮 × 3 轮 = 288:

| 策略 | hard 档提名 | llm 档提名(v1.1) | llm 档提名(v1.2) |
|---|---:|---:|---:|
| greedy | 288 | 288 | 288 |
...
**凡是 LLM 真正跑通的轮次,提名数从 96 塌到 ~5;凡是回退到确定性的轮次,提名满额 96。
无一例外。** v1.2 复现同样:两个策略唯一的 LLM 轮都只提名 5 个,第 2 轮已无候选可提,
campaign 在 `if len(picks) == 0: break` 处**静默终止,退出码 0**。

#### 因此 §7.11 的两条结论都不成立

1. **「接 LLM 后批次质量下降」——反了。** 按每次提名归一化:
   ...
   原始总数下降纯粹因为提名次数少了 36%–64%。**质量指标是升的。**

2. **「knowledge_agent(59)优于 agent_no_knowledge(32)」——是抽样假象。**
   knowledge_agent 拿到 197 次提名而无知识组只有 105 次,差别来自它多命中了
   一个满额确定性回退轮次,与知识库无关。

#### 那么真正的负面发现是什么(这条可以写,而且更重)

**不是「LLM 选得差」,是「LLM 一跑通就把候选空间打塌,且不稳定到能让实验静默终止」。**

- 确定性假设生成给出每位点 top-8 共 32 个替换 → 组合库 6,539 个候选 → 满额提名 96。
- LLM 假设生成每次只给出寥寥几个替换 → 组合库塌到个位数 → 提名 4–5 个。
- 两次独立运行的严重程度完全不同(105/197 vs 5/5),第二次直接死在第 1 轮之后。

这对试题「LLM Agent 是否真正学到了『科学家思维』」的回答比原来更硬:
**把 LLM 放进内核,它每次真正发言都会让实验的探索面塌掉,而且塌多少不可预测。**

#### 连带暴露的一个软件缺陷(尚未修)

`evolution/campaign.py` 会**把只花了 5 次预算的臂和花满 288 次的臂并排放进同一张对比表**,
不告警、不标注。`n_nominated` 字段一直都在,信息没丢,但没有任何东西提示读者
这两列不可比——§7.11 那个错误结论就是这么产生的。**修复方向:预算未花满时
在 metrics 与终端摘要里显式标注,并拒绝在同一张表里静默并列。**

#### 引用纪律

- 引用 llm 档任何数字时,**必须同时给出该臂的实际提名数**。
- 「同预算对比」这句话**不适用于 llm 档**,不得沿用。
- v1.1 与 v1.2 是同条件(`LLM_TIMEOUT=90`)的两次独立运行,可以并列作为
  「不稳定性」的证据;但**不得**取平均当作一个效应量。

数据:`lab/reports/workflow-v1.1/gb1/campaign_llm.metrics.json`、
`lab/reports/workflow-v1.2/gb1/campaign_llm.metrics.json`(各 `strategies.<策略>.rounds[].n_nominated`)。

### 7.15 【v0.8 2×2 结果·2026-09-13】残差反思不改变提名——因为工具契约装不下残差证据

> **2026-09-13 二次修订**:本节原标题是「LLM 从未行使过它唯一能改变批次的杠杆」,
> 那句话事实无误但**归因是错的**,读起来像在指责模型。查过工具契约后的准确归因见
> 下面「#### 为什么它不调用(根因,查代码得出)」。**请按修订后的措辞写,不要用原标题。**
>
> **本节的结论只在 v0.8 契约下成立。** §7.16 换掉契约后重跑了同一个 2×2,
> 同一模型、同一 seed、同一份注入内容,批次立刻分叉。**引用本节必须同时说明它测的是
> v0.8 契约**,否则会被读成「这个模型不会用证据」,那是错的。

**完整证据:`lab/tasks/task_91e931121d3ae400a0a2be246b-v0-8-aav/artifacts/factorial-2x2-seed42.md`。
写这一节前先读它的第 ⑤ 节引用纪律。**

2×2 因子实验(采集档位 × 残差反思,AAV,seed 42,288 预算,四格固定 `--backtrack semi`):
...
**这不是仪器失灵**:同一判据看得见采集档位造成的差异(v05 与 v06 的批次哈希完全不同),
只是看不见反思造成的差异。设计自带阳性对照。

**处理确实施加了**:v05 反思臂有 5 次注入、6007 字符真实残差内容,
且该臂比对照臂慢 20–25%——LLM 读了、处理了,然后做了同样的决定。四臂均 6/6 无超时。

#### 观测事实(硬,可独立复核)

四臂合计 **20 次 `compose_batch`,`allocation_source` 全是工具默认值,
`agent_requested` 出现 0 次**。也就是说:agent 从头到尾没有给过 `exploit_ratio` 一个显式值。

#### 为什么它不调用(根因,查代码得出)

**别把这条读成「模型偷懒」。** 逐条查过 `agent/auto_researcher.py` 之后,
它不调用是四层原因叠加,而其中三层是我们自己设的:

1. **提示词明确要求它别传。** v0.5 段落原文「**Prefer that default** unless measured
   evidence justifies an explicit ratio」,v0.6 段落「**Prefer that default** while the CV
   evidence is high」。每轮 round prompt 更直接——把调用模板写死成 `compose_batch(n=48)`,
   参数位上根本没有 `exploit_ratio`。**`agent_requested = 0` 首先是遵从指令的结果。**
2. **就算传了也会被地板夹掉,而且恰好夹死残差该触发的那个方向。**
   `_enforce_exploit_floor`:CV ≥ 0.80 → 利用比地板 0.80;最后两轮 → 地板 0.90。
   本轮实测自适应默认值为 r1 0.8707 / r2 0.9139 / r3 0.9721 / r4 1.0 / r6 1.0,地板 0.8→0.9。
   残差反思的自然反应是「看到致死 motif 就少赌预测、多探索」,即**调低** exploit_ratio,
   而地板规则**只禁止这一个方向**;第 4 轮之后可行区间是空集,这个动作数学上无法表达。
3. **杠杆与证据不同构(我认为这是根因)。** 注入卡片给的是 `N21D (over=3, under=0)`
   这种「哪些候选该排除」的信息;`exploit_ratio` 是个标量,控制「按预测均值取多少 /
   按多样性取多少」。**没有任何取值能表达「别选带 N21D 的」。** 卡片末尾那句
   `ACTION CONSTRAINT: avoid carrying them forward` 它想执行,手上没有入口:
   `compose_batch` 不收排除列表,`redirect_batch` 只按与最优簇的 mutation 重叠度跳。
4. **工作流关掉了最后一道口子。** compose → `test_composed_batch()`(无参数),
   并明令禁止把序列抄进 `test`(防转录损坏,本身是对的)。副作用是它连
   「手工剔掉这 3 个带致死 motif 的候选」都做不到,只能整批照单全测。

#### 最适合写进正文的一句

> 残差反思没有改变提名,不是因为反思无用,也不是因为模型不愿动——而是
> **这套工具契约里没有一个能承载残差证据的入口**。唯一的标量入口被提示词劝退,
> 并在campaign后半程被地板规则压成空集。它把致死 motif 写进了总结,
> 因为**叙述是它唯一能容纳这条证据的容器**。

这是对试题「LLM Agent 是否真正学到了『科学家思维』,还是只是调用预测模型」迄今最精确的回答,
但要害不在模型:**这套 agent 的「自主性」是被工具契约的形状决定的,不是被模型能力决定的。
在给它一个能表达证据的入口之前,「它会不会用证据」这个问题根本没有被测过。**
配套证据是自我叙述的不可靠性——第一轮冒烟里它写过 "Residual reflexion excluded the
two strongly lethal round-5 contexts",而那轮实测批次与对照臂逐字节相同。

#### 硬约束

- **n=1(seed 42)。** 同构型下「basin hop 提前到第 2 轮」在第一轮冒烟出现过、
  本轮**没有复现**——行为层单次观察不可信。
- **不要写成「反思无效」,也不要写成「agent 不愿意用工具」。** 准确说法:
  「在本 agent 的当前工具契约下,强制注入残差不改变其提名;因为契约里没有能表达
  该证据的参数,而唯一的标量参数被提示词劝退且被地板规则夹死。」
- 「批次逐位相同」与「20/20 未行使裁量」两条是硬事实,可独立复核,但都只覆盖 seed 42。
- **契约层归因(第 1/3/4 条)是读代码得出的结构事实,不依赖 seed**;第 2 条的
  「区间为空集」用的是本轮实测 CV,换 CV 轨迹结论可能不同。
- 「agent 请求了但被地板夹掉」与「agent 没请求」在数据上**是可区分的**:
  `allocation_source` 在夹之前就定为 `agent_requested`,且 `requested_exploit_ratio`
  与 `exploit_ratio` 分开记录。本轮观测到的是后者(压根没请求)。

#### 顺带:一个方法学缺陷,适合写进「工程严谨性」

**第一次 2×2 是无效的。** 采集策略挂在 `backtrack` 上的地方有两处(提示词段落、
`compose_batch` 工具默认),第一次只解耦了提示词层。四个臂命令行看起来是 2×2,
实际 `allocation_source` 全是 `v06_pure_exploit_default`、批次逐位相同——
提示词告诉 agent「用自适应规则」,工具照样返回纯利用。
**只修一层比两层都不修更危险:实验看起来是受控的,其实不是。**
已加双层守护测试并反向验证;比对脚本改为取事件流里的实测档位而非命令行意图。

### 7.16 【v0.9 2×2 结果·2026-09-13】换掉契约后,同一个模型的提名立刻分叉

**完整证据:`lab/tasks/task_feb731c779788be5bfff65e979-v0-9/artifacts/contract-2x2-seed42.md`。
写这一节前先读它的第 ⑥ 节引用纪律。**

§7.15 定位出「反思无效」的根因是工具契约装不下残差证据。v0.9 改造契约后重跑同一个 2×2
(工具契约 × 残差反思,AAV,seed 42,288 预算,四格固定 `--backtrack semi --acquisition v05`):

| 臂 | 峰 | strong | 自设 ratio | 用排除入口 | redirect | LLM | 秒 |
|---|---:|---:|---|---|---|---|---:|
| v08 契约 · 反思关 | 8.4162 | 158 | **0/5** | 0/5 | `[5]` | 6/6 | 238 |
| v08 契约 · 反思开 | 8.4162 | 158 | **0/5** | 0/5 | `[5]` | 6/6 | 266 |
| v09 契约 · 反思关 | 8.4162 | 153 | **5/5** | 0/5 | `[5]` | 6/6 | 527 |
| v09 契约 · 反思开 | 8.4162 | **160** | **6/6** | **5/6** | **`[]`** | 6/6 | 573 |

| 契约 | 反思开 vs 关 |
|---|---|
| v0.8 | 批次未分叉(r1–r6 全同,复现 §7.15) |
| v0.9 | **批次已分叉**,r1 46/48 · r2 42/48 · r3 37/48 · r4 25/48 · r5 26/48 · **r6 8/48** |

**分歧是渐进发散的**,不是一次性跳变——累积学习该有的形状。

#### 三条把结论钉住的检查

1. **对照臂逐位复现 v0.8。** `contract-v08_reflexion-off` 的六轮批次哈希与 v0.8 的
   `acq-v05_reflexion-off` 完全相同。这是在真实 LLM 跑批下证明代码改动没污染对照。
2. **排除的 motif 全部来自证据,零凭空。** 逐轮核对「被 `exclude_motifs` 排除的替换」
   与「注入卡片里 `over>0` 的替换」:排除集 ⊆ 累积卡片证据集,**零例外**;
   且全程卡片标了 34 个,它只排了 **20** 个——有选择,不是一刀切。
   排除集单调增长 0→2→4→8→13→20,证据在跨轮累积。
3. **四臂均 288/288 花满预算、6/6 LLM、0 超时、0 工具错误。**

#### 最适合写进正文的两句

> 残差反思在 v0.8 契约下对提名零影响;换上一个能表达该证据的契约后,**同一个模型、
> 同一个 seed、同一份注入内容**,批次立刻分叉并逐轮发散(第 6 轮只剩 8/48 相同)。
> 所以 v0.8 测到的不是「这个模型不会用证据」,是「这套契约装不下证据」。

> 打开契约后 `agent_requested` 从 **0/20** 变成 **11/11**,排除入口 6 轮用了 5 轮,
> 每一个被排除的替换都能在此前注入的残差证据里找到出处。
> **agent 的「自主性」是被工具契约的形状决定的,不是被模型能力决定的。**

#### 交互效应的形状(数字很小,只能当方向)

v08(无方向盘)158 → v09 反思关(**有方向盘、无证据**)**153** → v09 反思开
(**方向盘 + 证据**)**160**。形状是「单给方向盘反而略差,配上证据才变好」,
支持「自主性只有在有证据可依时才有价值」。**差值只有 5–7 个 strong、n=1,
绝不可当效应量引用。** 峰值四臂全部 8.4162,「9.536 悖论」照旧。

#### 硬约束

- **n=1(seed 42)。** 批次哈希类断言是确定性事实;**行为类断言(v09 反思臂是唯一
  没触发 basin hop 的)是单次观察,不可信。**
- **不要写成「反思有效」。** 准确说法:「在 v0.9 契约下注入残差改变了该 agent 的提名,
  且改变方向可追溯到证据本身;在 v0.8 契约下同样的注入零影响。」
- **v0.8 的结论没被推翻,是被限定了适用范围。** 引用 §7.15 时必须同时说明它测的是 v0.8 契约。
- **契约是复合处理**(提示词中性化 + 地板释放 + 排除入口),本轮**没有拆开**测哪一样
  贡献最大。不得声称「是排除入口起的作用」,只能说这三样一起起了作用。
- **v09 臂慢一倍**(527/573s vs 238/266s)。论成本时这是真实代价。
