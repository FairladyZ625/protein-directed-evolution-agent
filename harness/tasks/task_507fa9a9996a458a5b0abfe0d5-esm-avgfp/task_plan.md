...
在池式引擎上,让四策略对比在"更难数据集 + 校准的不确定性"下如实呈现。产出:AAV(ESM-2)与 avGFP(≥1 特征)的 `reports/pool_metrics_*.json` + 事件流 + 曲线,以及一份 `harness/tasks/<本任务>/artifacts/harder-findings.md` 记录跨数据集结论(诚实:agent/知识到底比 greedy 强不强)。
...
3. `harness/context/development/ai4s-worker-handbook.md`(若存在)+ 本仓 CLAUDE.md/AGENTS.md 纪律。
