...
**障碍二:没有一处能看到全部版本。** v0.5 只在 `.worktrees/t-v05`,v0.6 只在 `.worktrees/t-v06`,v0.7 只在 `.worktrees/t-v07`,主树 `lab/reports/` 只有 v0.1–v0.4。三个 worktree 没有任何一个同时拥有 v0.5+v0.6+v0.7。
...
3. **全版本汇总进主树**:把 v0.5/v0.6/v0.7 的 report 与产物从各自 worktree 收进主树 `lab/reports/`,使一处可见全部 7 个版本。
4. **产出版本矩阵总表** `lab/reports/version-matrix/report.md`:每个版本一行,列出协议(轮数/每轮/总预算/实花)、surrogate、采集策略、是否用 LLM、cum_top10_max、strong、是否达峰,并**明确标注哪些数是原始记录、哪些是本次统一复跑**。
...
3. `.worktrees/t-v05|t-v06|t-v07/lab/reports/agentic-v0.5|0.6|0.7/report.md`(**权威**:待汇总的三个版本的原始结论,汇总时不得改写)。
4. `lab/context/research/v07-multiseed-robustness.md`(**权威**:v0.7 的 30seed×7方法口径,总表里 v0.7 一行以它为准)。
5. `lab/context/research/v06-backtrack-analysis.md`(**权威**:v0.4 "deterministic" 实为交替的证据)。
6. `lab/reports/agentic-v0.1|v0.2|v0.4/report.md`(原始叙事,**只读**,不得覆盖)。
...
- **并发(硬约束)**:同期在飞——包A(`task_0bc2d4d678eea24c4930b60a0e`,动 `evolution/campaign.py`/`agent/pipeline.py`/`agent/llm.py`/`app/demo.py`)、包C(`task_6d1dc285d32628ed317b9bb4da`,**正在改 `lab/reports/**` 里 v0.1–v0.4 的模型标签与勘误段**)、知识增强实验(`task_25092f640504bf3b040f6adfb5`,动 `knowledge/**` 与 `agent/auto_researcher.py`)。

## Execution Surface

- 分支 `t-version-matrix`;dispatcher 注入 cwd。
- **允许写**:`lab/reports/version-matrix/**`(本任务的新产物与总表)、`lab/reports/agentic-v0.5/**`、`lab/reports/agentic-v0.6/**`、`lab/reports/agentic-v0.7/**`(从 worktree 汇总进主树的**新目录**)、本 worktree 内的复跑脚本。
- **禁区(极重要)**:
  - `lab/reports/agentic-v0.1/**`、`v0.2/**`、`v0.3/**`、`v0.4/**` —— **包C 正在改这些文件**。复跑结果一律写进 `version-matrix/`,**不要覆盖或修改任何历史版本目录**。保留原始记录本身就是诚实性证据。
...
- **第二次停**:汇总完成后报一句,主树 `lab/reports/` 下是否已能看到全部 7 个版本。
...
- 写 `lab/reports/version-matrix/report.md` 总表。
- `ha task progress append` 留痕;`ha fact record --task task_dff265c794250339f712ebcdce` 记承重结论。

## Deliverable Contract

- `lab/reports/version-matrix/report.md`:预注册参数、版本矩阵总表(原始记录 vs 统一复跑两列)、每条的 seed 方差、诚实判定(统一协议后版本排序有没有变)。
- `lab/reports/version-matrix/` 下的指标 JSON 与事件链(新 schema,含逐轮全部提名及 HD)。
- 主树 `lab/reports/agentic-v0.5|v0.6|v0.7/` 齐全。
