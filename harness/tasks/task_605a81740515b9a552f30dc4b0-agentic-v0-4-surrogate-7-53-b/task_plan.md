...
在 AAV 池、同 cold-start(HD≤2)、同门内定义(HD≤4 且平均 BLOSUM62≥0,9533)下,产出**上位感知 surrogate 的确定性诊断**:真峰门内排名 + pool/gate Spearman,与实验二加性基线(esm2×knn #1283、one_hot×ridge #2776)对照。判据:是否有上位感知 surrogate 把真峰排进 ≤288 可达区。若诊断为正,再接**闭环跑分**(同 288 预算、同门禁、同采集,仅换 surrogate)产出 agentic v0.4 的 cum_top10_max,看是否突破 7.53。全程 answer-agnostic(不看测试峰成分;位点/特征选择不用峰信息)。交付:`harness/reports/agentic-v0.4/aav/` 诊断 JSON(+ 若跑闭环则 metrics/events/figure)+ 报告 report.md。
...
3. `harness/reports/agentic-v0.3/report.md` + `aav/surrogate_scan.json`(加性基线数字)。
4. `harness/context/research/plateau-breaking-methods.md`(族 2 方法与提名 B/A 依据)。
...
仓库根。写范围:`models/train_ladder.py`(加上位感知 surrogate)、`analysis/`(诊断脚本,可扩展 representation_surrogate_scan 或新建 epistasis_surrogate_scan.py)、可补 `tests/`(surrogate 接口单测)、`agent/auto_researcher.py`(仅当接闭环:扩 `--surrogate` 选项);产物落 `harness/reports/agentic-v0.4/`。不改 workflow 线、不动 v0.1–v0.3 产物。
...
2. 诊断脚本(扩 `analysis/representation_surrogate_scan.py` 或新建):对 {加性基线, 上位感知} 各算门内真峰排名 + pool/gate Spearman + top-288 命中,输出 JSON 到 `harness/reports/agentic-v0.4/aav/`。
...
- 诊断脚本 + `harness/reports/agentic-v0.4/aav/epistasis_surrogate_scan.json`。
