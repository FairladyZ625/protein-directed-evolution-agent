...
1. `lab/context/research/AI4S-assignment.md`（题目三详细要求：报告 8 章目录、代码/README 要求、结果展示清单）— 权威：题面（逐条对齐）。
2. T7 `reports/campaign_metrics.json` 与曲线、T3 指标表 — 权威：实验数据源。
3. `lab/context/architecture/AI4S-master-plan.html`（报告 8 章对位、砍单顺序）— 权威：设计。
