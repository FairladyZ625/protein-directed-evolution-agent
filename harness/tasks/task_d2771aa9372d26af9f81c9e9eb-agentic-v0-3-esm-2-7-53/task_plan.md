...
3. `harness/reports/agentic-v0.1/aav/postmortem_esm.json`(权威:ESM 预测器技能 + 真峰全池排名 #5759)。
4. `harness/reports/agentic-v0.2/report.md`(权威:v0.2 结论与 7.53 天花板机制)。
5. `evolution/datasets.py` 的 `esm_encoder`/`load_aav`(参考:ESM 缓存机制,`harness/reports/cache/esm_aav.npz` 已覆盖全 38265 序列)。
6. `features/esm2.py`(参考:ESM2Embedder,权重机器本地 650M)。

## Entry Conditions

- `harness/reports/cache/esm_aav.npz` 存在且覆盖全池(实测:38265×1280,已满足)——首跑为缓存命中的快跑,不需 650M 重提。
...
仓库根(dispatcher 注入 cwd)。写范围:`models/train_ladder.py`(加代理模型,不动 RidgePredictor 现有行为)、`agent/auto_researcher.py`(加 `--surrogate` 选项选代理,默认 ridge 保持 v0.1/v0.2 行为)、可补 `analysis/`(表征×代理 排名诊断脚本)+ `tests/`(新代理接口单测);产物落 `harness/reports/agentic-v0.3/`。不改 workflow 线、不动 v0.1/v0.2 产物。
...
6. 入 ledger,写 `harness/reports/agentic-v0.3/report.md`(四方对比 + 诊断表)+ manifest.json + figure。
...
- `tests/` 新代理单测 + 门禁回归通过。
- `harness/reports/agentic-v0.3/aav/`:agentic.metrics.json + events.jsonl(链校验)+ figure;`harness/reports/agentic-v0.3/report.md`(四方对比)+ manifest.json。
