...
- `EventStore('harness/reports/experiment_log.jsonl').verify()` passed after nine append-only backfill records (24 records total).
...
for manifest in Path('harness/reports').glob('*/manifest.json'):
