...
在 AAV 池、同预算(288)、同 oracle 下,产出 agentic v0.2 跑分并与 v0.1(cum_top10_max 5.96)、greedy(7.53)对照:验证"test 前的 HD≤4 + 平均 BLOSUM62≥0 硬门(拒收不扣预算、返结构化错误让 agent 重提)"是否把探索从"选中死蛋白"扳回有效区。交付物形态:`harness/reports/agentic-v0.2/aav/` 下的 metrics/events(链校验)/figure + 中期报告 report.md;收件人 CEO 与泽宇;首个使用者是最终报告的"知识增强对 agentic 是否有效"章节。
...
2. `harness/reports/agentic-v0.1/report.md` + `aav/postmortem_*.json`(权威:v0.1 失败机制与基线数字)。
...
仓库根(dispatcher 注入 cwd);写范围仅 `agent/auto_researcher.py`(加门禁参数与逻辑)+ 可补 `tests/`(门禁单测)+ 产物落 `harness/reports/agentic-v0.2/`。不改 workflow 线代码、不动 v0.1 产物。
...
- 跑 v0.2 LLM 版,入 ledger,写 `harness/reports/agentic-v0.2/report.md` 三方对比。
...
- `tests/` 门禁单测通过。
- `harness/reports/agentic-v0.2/aav/`:agentic.metrics.json + events.jsonl(链校验)+ figure;`harness/reports/agentic-v0.2/report.md`(v0.2 vs v0.1 vs greedy)+ manifest.json。
