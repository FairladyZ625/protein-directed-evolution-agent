...
交付 `lab/context/research/v06-backtrack-analysis.md`(278 行,含可复跑的内嵌 runner 脚本):对 v0.6 "LLM 元层自主 backtrack/redirect"的对抗性分析——停滞是怎么产生的、backtrack 能不能达峰、redirect 判据该用"纯高均值"还是"换构成"。
...
- 文档实存并已 canonical 登记:`lab/context/research/v06-backtrack-analysis.md`。
- **本任务的现行事实是 `F-ED1ED1BE`,它取代了 `F-6D7255DE`**(`ha fact record --supersedes fact/F-6D7255DE`,2026-09-14)。取代的原因正是独立评审指出的那一点:`F-6D7255DE` 的 Evidence source 写成 `artifacts/context/research/v06-backtrack-analysis.md`,该路径只是任务包内的一份未跟踪副本;`F-ED1ED1BE` 已改指 `lab/tasks/task_e2eab881832291e8f8f73f30a4-v0-6-llm-backtrack-redirect-vs/artifacts/v06-backtrack-analysis.md`,与本 closeout 的交付锚点 `artifact:artifacts/v06-backtrack-analysis.md` 为同一文件。
- **须同时说明一个读面缺陷,否则这条血缘无法自证**:fact 投影不渲染 supersede 关系。全仓 132 个 fact 文件**全部**是 `State: standing`,`lab/facts/` 下 "supersed" 字样命中数为 **0**,取代者自身也没有 `Supersedes:` 字段。指名对照:`F-5A4B6556` 已被 `F-E9C38438` 明确取代(后者正文直接写前者结论「为假」),投影里仍显示 `standing`。因此评审员读到「`F-6D7255DE` 仍 standing 且路径错」并判证据链不可接受,是在一个有损读面上做出的合理判断,不是误判;修复投影属框架侧,本任务只能在正文显式声明血缘。
