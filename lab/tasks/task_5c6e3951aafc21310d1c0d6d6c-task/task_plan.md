...
1. **两棵并行的报告树**:根 `reports/`(149 个文件)与 `lab/reports/`(141 个文件)。
   而 `evolution/results_layout.py` 的模块 docstring 明确声明 canonical 位置是
   `lab/reports/<line>-v<version>/`,根 `reports/` 是历史遗留。评委会看到两份看似
   都权威的产物树,无从判断该信哪个。
2. **根目录散落物**:`figures/`、`tmp/` 直接躺在仓库根。
3. **数据脊柱残缺**:`lab/reports/experiment_log.jsonl` 只有 15 条,
...
- `lab/reports/experiment_log.jsonl`(看现有 15 条的字段形状,照它补)
...
无阻塞。注意 `lab/reports/workflow-v1.1/` 下 CEO 正在重跑四策略,**那个目录别动**。

## Execution Surface

可动:根 `reports/` 的收敛、`figures/`、`tmp/`、`.gitignore`、
`lab/reports/experiment_log.jsonl` 的补全、各版本缺失的 `manifest.json`、
`README.md` 里指向产物位置的说明。

**不要动** `lab/reports/workflow-v1.1/`(CEO 在跑)、`lab/reports/agentic-v0.*/`
与 `final-report-v0.*/` 里**已有的数据文件内容**(可以加 manifest,不可改数据)、
`evolution/`、`agent/`、`models/`、`knowledge/`、`events/`、`app/`。

## Constraints

- **数据只增不删(红线)**。根 `reports/` 里若有 `lab/reports/` 没有的产物,
...
- 脊柱补全的每条记录里,`git_commit` 必须是**真的产出该产物的那个 commit**;
  ...
  **不许填一个看起来合理的 commit**。
- 不许为了让目录变整齐而删除任何 `lab/facts/`、`lab/decisions/`、
  `lab/tasks/` 下的内容。

## Checkpoint

若发现根 `reports/` 与 `lab/reports/` 存在**内容冲突**(同名产物但 SHA-256 不同),
...
1. 全量列出根 `reports/` 与 `lab/reports/` 的文件与 SHA-256,做差集。
2. 差集里只在根 `reports/` 有的 → 迁入 `lab/reports/` 对应版本目录(按内容判断归属;
   判断不了的回报)。
...
- 收敛后的单一产物树。
- `lab/reports/migration-audit.md` —— 迁移前后的文件清单 + SHA-256 对照表,
  明确写出「迁移了几个、丢失 0 个」。
...
- 迁移后 `find lab/reports -type f | wc -l` 的数字要与对照表一致。
