...
3. **复现脊柱补全**:`lab/reports/experiment_log.jsonl` 覆盖到所有已产出的实验;
...
- **两棵产物树并存,这是合规的不是缺陷**:根 `reports/` 是早期扁平产物 + 用户正在写的
  `reports/final-report-v0.5/`(**这个目录任何人都不许动**);`lab/reports/<方法线>-v<版本>/`
  是版本化树。`lab/reports/migration-audit.md` 记录了迁移审计。
...
| `lab/reports/experiment_log.jsonl` | 脊柱的现有格式;**照它的 schema 追加,不要另立新格式** |
| `lab/reports/workflow-v1.0/manifest.json` | manifest 的现有形状,新建的要同形 |
| `lab/reports/migration-audit.md` | 两棵树的由来,避免你把合规的并存当成要收敛的缺陷 |
...
- **注意**:CEO 正在并行跑 `workflow-v1.1` 的 hard/sparse/llm 三个 regime,产物会陆续落到
  `lab/reports/workflow-v1.1/gb1/`。**你不要动那个目录下的 campaign 产物文件**;
...
`lab/reports/experiment_log.jsonl`(只追加)、各版本目录的 `manifest.json`(只新建缺的)。

**禁区**:
- `reports/final-report-v0.5/`(用户正在写的报告,**绝对不动**)
- `app/demo.py`、`tests/test_demo_app.py`(前端包在飞)
- `lab/reports/workflow-v1.1/gb1/campaign_*.{json,jsonl,png}`(CEO 在跑)
...
2. 某个历史产物的来源命令查不到(去 `lab/tasks/*/artifacts/` 和 git log 找过之后仍无)。
...
- 七个新 `manifest.json`。
- 一份 `lab/tasks/<本任务>/artifacts/delivery-verification.md`:
