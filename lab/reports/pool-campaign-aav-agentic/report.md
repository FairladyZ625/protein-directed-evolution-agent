...
- 若要在报告里提及,只能作为「失败案例」或「工程缺陷现场」引用,
  并且必须写明 `n_rounds = 0`。
- AAV 上真正可引用的 agentic 结果在 `lab/reports/agentic-v0.1` 至 `agentic-v0.7`,
  知识增强对照在 `lab/reports/knowledge-ablation/`。

## 校验

```bash
python3 -c "
import json;d=json.load(open('lab/reports/pool-campaign-aav-agentic/pool_metrics_aav_agentic.json'))
