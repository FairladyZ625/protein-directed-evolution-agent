...
VERSIONS = {
    ...
    'v0.7': (
        ...
        '确定性交替采样策略在第 4 批次精确命中 8.4162 未测候选池全局真峰。进一步系统参数网格扫描证实，重探索 UCB 采集函数（β=3）在固定候选池上确定性达峰 8.4162——该配置不消耗采集随机数，30 个种子跑出的是同一条轨迹，因此这是确定性结果而非 30 次独立成功（真正含随机成分的交替调度与 Thompson 采样达峰率为 2/30 与 3/30）。据此确立微观确定性数学优化与宏观智能体战略调度的双环系统架构。',
...
EXECUTIVE_SUMMARY = [
    {
        ...
        'highlight': '纯 LLM 停滞于 7.8290 vs UCB (β=3) 确定性达峰 8.4162',
        'data_contrast': (
            '• 纯大模型自主决策：深陷 <b>7.8290</b> 局部鞍点停滞<br>'
            '• 重探索 UCB (β=3)：<b>确定性达峰 8.4162</b>（30 种子同一轨迹，非独立重复）<br>'
            '• 探索税定量测定：<b>Δ = 0.5872</b>（8.4162 − 7.8290）'
        ),
        'text': (
            '在 288 次受限微观采样中，纯大语言模型无法精准估算高维非凸能量面的连续梯度与认知方差，极易陷入局部鞍点停滞；'
            '而引入置信上界的贝叶斯优化算子（UCB β=3）在固定候选池上确定性收敛至真峰；该路径不消耗采集随机数，30 个种子跑的是同一条轨迹，故为确定性结论，不是 30 次独立试验的成功率。'
    ...
    },
    {
        ...
        'data_contrast': (
            ...
            '• 内环确定性算法保障：微观采样<b>确定性收敛</b>至池内真峰<br>'
...
RESEARCH_INSIGHTS = [
    {
        ...
        'bg': '#fff1f2',
        'border': '#fecdd3',
        'summary': '在 288 步受限微观采样预算下，纯大语言模型在连续能量面寻优中陷入 7.8290 局部鞍点停滞；而确定性 UCB 采集函数（β=3）在固定候选池上收敛至 8.4162 全局真峰（30 个种子同一轨迹，非独立重复）。该实证构成高度可信的干净负结果，定量确认了连续参数空间并非自回归大语言模型的优势场域。',
        'core_data': [
            ...
            {'label': '重探索 UCB (β=3) 达峰', 'val': '确定性达峰', 'desc': '该配置不消耗采集随机数，30 个种子跑出同一条轨迹（第 3 轮达峰），非 30 次独立成功；纯均值贪心 (β=0) 同为确定性、始终不达峰。含随机成分的交替调度为 2/30、高斯 Thompson 为 3/30'},
...
OUTER_LOOP = {
    ...
    'v0.7': [
        ...
        },
        {
            'kind': 'signal',
            'title': '纯贪心鞍点停滞对照验证',
            'detail': '纯均值贪心在 3 个独立随机种子下均于第 2 轮停滞于 7.8290 局部鞍点；而 30-seed 网格扫描确证 UCB (β=3) 确定性收敛达峰（30 种子同一轨迹）。',
...
COGNITION_ARC = [
    ...
    },
    {
        ...
        'detail': '通过 7 方法 × 30 种子网格扫描：重探索 UCB 采集函数于第 3 轮达峰，纯贪心对照始终不达峰。需同时说明口径——两者均为确定性配置，30 次跑的是同一条轨迹，不能按独立样本读；含随机成分的交替调度与 Thompson 采样达峰率分别为 2/30 与 3/30。',
        'ref': 'reports/explainer_for_humans.html #sec8',
    },
]
...
HONEST_BOUNDARIES = [
    {
        ...
        'detail': '均值与 UCB 路径不消耗采集随机数，在固定候选池上 30 个种子跑出的是同一条轨迹，'
                  ...
                  '本项目中真正含随机成分的只有交替调度与高斯 Thompson 采样（各 30 条不同轨迹），其达峰率为 2/30 与 3/30。',
    },
    {
