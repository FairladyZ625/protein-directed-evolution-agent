...
(async()=>{
 ...
 await page.goto('file://'+path.join(root,'scientific_report_v0.5_two_column.html'));
 ...
 await page.pdf({path:path.join(root,'scientific_report_v0.5_two_column.pdf'),preferCSSPageSize:true,printBackground:true});
 await browser.close();
})();
