...
产出 `lab/context/research/assignment-coverage-audit.md`:一张**逐条对照表**,试题里每一个可验收的要求一行,列:
...
- 试题原文镜像:`lab/context/research/AI4S-assignment.md`(权威原件是明度数智的 docx)。
...
- 主要交付面:`evolution/`、`models/`、`features/`、`knowledge/`、`agent/`、`app/demo.py`、`analysis/`、`reports/`、`lab/reports/agentic-v0.*/`。
- 注意版本时效:最终科学定性以 fact `F-885537A3` 为准(两目标两策略),早期报告的结论已被取代——但**本任务审的是"要求有没有做",不是"结论对不对"**,别混。

## Required Reading

1. `lab/context/research/AI4S-assignment.md`(**最高权威**:被审标尺,逐字读完)。
...
4. `lab/context/research/v07-consolidated-summary.md`(已有成果全貌,避免把做过的判成没做)。
...
- **允许写**:仅 `lab/context/research/assignment-coverage-audit.md`(+ 自己 worktree 内的临时核查脚本,不进产品树)。
...
- `lab/context/research/assignment-coverage-audit.md`:逐条对照表 + 三张汇总表 + 优先级补齐清单。
- ≥1 条 fact。
