...
先读/Users/lizeyu/.claude/skills/fable-gpt-worker-orchestration/references/codex-worker-handbook.md；harness/context/development/ai4s-worker-handbook.md；本task_plan；app/demo.py（重点render_alpha_sweep_panel及五个tab）；harness/reports/REPORT-HANDOFF.md按图表需要路由；用户要求本轮只读，优先于身份旧实现指令。
