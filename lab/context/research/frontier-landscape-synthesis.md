...
> **⚠️ 勘误(2026-09-12,fact F-8514C714,supersedes F-EE89324D)**:本文档 §2 共识B、§4 对大问题的回答里"greedy 达峰、benchmark 偏易、探索有毒、去自主化是解药"的结论**在机制上是错的**,源于 v0.4 报告把一个 mean/diverse **交替**基线误标成"纯 greedy"。经 CEO 独立复现:纯利用(真 greedy)6 轮**封顶 7.829**,真峰 8.4162 在 cold-start surrogate 均值排名 **#2758(对利用隐形)**;8.416 是被**探索(diverse)轮**捞出来的。**更正后的结论:纯贪心够不到真峰;达峰必须靠探索;本 benchmark 不偏易(最优变体对模型隐形=真实 DE 核心难点);LLM 的真短板是探索质量(输给朴素 diverse),不是利用不足。** 下方 §2-§5 未改写的"greedy 达峰"表述以本勘误 + F-8514C714 为准。
...
- 第①路 GLM:`lab/context/research/v05-agent-optimization-glm.md`
- 第②路 Gemini:`lab/context/research/v05-agent-optimization-gemini.md`(516 行,含四病灶事件流诊断 + β 退火公式 + 四层防御)
...
- 本仓实测:`lab/reports/agentic-v0.1..v0.4/report.md`、`plateau-breaking-methods.md`
