...
交付提交 `646ad491646005cc98dc48353fcfdcbc7b73656f`(结果与文档)。实现先落在同一执行的前一个
...
> **交付切面迁移(2026-09-14,依据独立评审 `review_de_reviewer_20260914_v09_r3`)**:本任务原先钉在 `35915419`。独立评审用 `git ls-tree` 证明该提交**不含** `exclusion_too_strict` 守护测试(它在 `646ad49`),也不含 plan 声明的 `tmp/v09-contract/`,因此那个 content-pin 撑不起本 closeout 的断言——这一点成立。现改钉 `646ad49`(`fix(test): 收集范围钉在 tests/,补 exclusion_too_strict 覆盖`),已核实:该提交树同时含 `pytest.ini`(+12) 与 `tests/test_auto_researcher_contract.py`(+28)、`exclusion_too_strict` 在其中命中 2 处、它是 `origin/main` 的祖先、且在 main 上有自己的绿色运行 `ci / completed / success / run=34830213086`——故 `ci` 完成门在本切面上有对齐证人。

## Verification

- `PYTHONPATH=. .venv/bin/python -m pytest -q` → 当时 **152 passed**。**该数字此后一度无法复现,原因与本任务无关**:`fbfd5c6`(2026-09-13 14:41)交付 v0.7 证据包时,把 `reports/final-report-v0.7/evidence/code-v09/tests/` 下三个与 `tests/` 完全同名的文件(`test_auto_researcher_backtrack.py`、`test_auto_researcher_contract.py`、`test_reflexion.py`)带进了收集范围;两边都无 `__init__.py`,pytest 按 basename 导入撞名,把**两边**都判成 collection error,于是全量变成 4 errors 起不来,而 CI 只点名跑 8 个自包含文件,没有任何门发现它(fact `F-89963893` 那条机制的第四次发作)。已于 2026-09-14 加 `pytest.ini` 把收集范围钉在 `tests/`(证据副本是交付物,不动它),全量恢复为 **166 passed in 28.62s**——该次测量时 `tests/test_auto_researcher_contract.py` 仍是 15 条;**补入第 16 条 `exclusion_too_strict` 用例之后全量为 167**:2026-09-14 实测 `167 passed, 1 warning in 31.93s`,按文件收集数逐项相加亦为 167(21 个文件,`git status --porcelain tests/` 为空、无新增文件)。独立评审报的 167 与当前状态一致;本 closeout 原先只写 166,是漏记了自己新增的那条用例,现把两个数字连同各自前提一并保留,不以后者覆盖前者。
...
- **声明测试路径已更正**:`task_plan.md` 的 Execution Surface 与 Verification 原先两处写 `tests/test_auto_researcher.py`,该文件**从未存在**——`tests/` 下实际是 `test_auto_researcher_backtrack.py`、`test_auto_researcher_compose.py`、`test_auto_researcher_contract.py`、`test_auto_researcher_gate.py` 四个。独立评审指出该声明路径不存在,成立;已于 2026-09-14 把两处改为本任务守护测试的真实落点 `tests/test_auto_researcher_contract.py`(实测 16 passed)。
...
- **`tmp/v09-contract` 旧落点的全仓范围与处置判断(2026-09-14)**。独立评审 r4 的阻断缺陷①②指向「提交树里的任务声明与 artifact 仍写 `tmp/v09-contract`」。全仓实测命中 22 处,按性质分三类,处置不同:
  - **应改且已改**:`lab/tasks/.../artifacts/contract-2x2-seed42.md`(2 处,已改为 `lab/reports/v09-contract` 并 `ha doc sync` 落库)、`lab/tasks/.../task_plan.md`(3 处,先前已改)、`scripts/run_v09_contract.sh`(已提交 `01f6822`)、`app/demo.py:1322`(**同一条 `st.warning` 内目录写 `lab/reports/v09-contract/` 而复现命令写 `tmp/v09-contract`,自相矛盾**,照抄会跑错落点,已提交修复)。
  - **不应改(历史取证记录,改即篡改)**:`reports/final-report-v0.7/report.md:1178-1191` 共 8 处,位于证据索引表——左列是归档后路径 `v09/...`,右列是**当时的原始产出位置** `tmp/v09-contract/...`,该表的作用正是记录「产物当时落在哪」;把右列改成新落点会使索引与历史事实不符。
  - **不应改(报告随附的代码快照)**:`reports/final-report-v0.7/evidence/code-v09/scripts/run_v09_contract.sh` 与 `evidence/code-metrics/run_v09_contract.sh` 各 2 处,是报告交付时脚本状态的冻结副本,反映当时默认落点确实是 `tmp/v09-contract`;改它等于伪造快照。活脚本 `scripts/run_v09_contract.sh` 已单独修好。
  - **无法改(daemon 单写的生成物)**:`executions/exe_88c8c3ac08cfe7d768f0427de1.md`、`reviews/review_de_reviewer_20260914_v09*.md`、`lab/facts/F-58B990D8.md` 等共 4 处,是台账生成的不可变记录,本就不应手工编辑。
- **原写「仍未解决的结构性阻塞,须 CEO 裁定:本仓命令面不存在 daemon 自动 git 提交通道」为假,2026-09-14 更正**:该通道一直存在——`harness/` 是**嵌套 git 仓**(`harness/.git`),由 `Harness SQLite Outbox <harness-sqlite-outbox@local.invalid>` 自动提交,提交信息形如 `harness sqlite outbox outbox-<revision>`,与 `ha settings read` 的 `entityRevision` 对应;该仓共 1878 个提交、**2026-09-14 当日 422 个**,最新为 `b7c84a6`(13:42:06,`outbox-2936`),而 `outbox-2936` 正是把 `reviewReturnBudget` 改为 10 那次写入的修订号。经 `ha doc sync --submit` 的文档在**嵌套仓内状态为已提交/无差异**。**我此前的错因**:只在主仓 `git status` 里看(主仓另外跟踪 harness/ 下 1582 个文件,故那一侧长期显示 `M`),并只查了 `ha daemon --help` 的子命令列表,**从未查过 `harness/.git`**;据此把回执里的 `git: pending` 误读为「没有任何 git 提交」。`git: pending` 实指**主仓**一侧未提交,与嵌套仓的 outbox 提交是两件事。**因此本条不构成阻塞,原先请求 CEO 授权手动提交的上报作废。**
