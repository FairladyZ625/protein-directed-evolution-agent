...
### 7.15 【v0.8 2×2 结果·2026-09-13】LLM 从未行使过它唯一能改变批次的杠杆
...
> 这个 agent 从未行使过它唯一能改变批次构成的杠杆。它调用工具、阅读被强制注入的
> 残差证据、在总结里讨论那些致死 motif,然后把采集比例原样交给工具默认值。

所以残差反思没有改变提名,**不是因为反思无用,而是因为这个 agent 不通过调整采集来响应证据**;
它对证据的反应停留在叙述层。这是对试题「LLM Agent 是否真正学到了『科学家思维』,
还是只是调用预测模型」迄今最精确的回答:**LLM 的贡献可以用哈希证明为零,
而它的自我叙述会声称相反的事**(第一轮冒烟里它写过 "Residual reflexion excluded the
...
- **n=1(seed 42)。** 同构型下「basin hop 提前到第 2 轮」在第一轮冒烟出现过、
  本轮**没有复现**——行为层单次观察不可信。
- **不要写成「反思无效」**。准确说法:「在本 agent 的当前工具契约下,强制注入残差
  不改变其提名;原因是它从不通过调整采集比例来响应证据。」换一个会主动设定
  `exploit_ratio` 的 agent,结论可能不同。
