...
```
$ ha doc sync --dry-run --path harness/context/research/x.md
error code=document_not_found
hint=harness/context/research/x.md is daemon-managed; use doc-sync: selected doc-sync path
...
$ grep '^- State:' harness/facts/F-412E158B.md  →  - State: standing
```

`F-6F717548` 同样。**fact 已被 supersede，但磁盘上的 fact 文件仍自称 standing。**
任何读文件而不走 CLI 的人(包括 agent)会把废除的事实当现行证据引用。本会话见到 2 个实例。

**相关**:fact 文件里**不含 taskId**，归属只存在于关系投影。我 `grep harness/facts/` 查不到归属，
...
事实与产物分布在至少四处:`harness/reports/`、`harness/context/research/`、`harness/facts/`、
...
**后果**:我判断一个关键数字「无产物支撑」、准备删掉它时，只搜了 `harness/reports/` 一个树。
真源(210 条完整矩阵、25 个文件全部 git 跟踪、自带复现命令)在 `harness/context/research/` 里;
**台账里甚至早有一条现行 fact 明文记着同一组数字**，而我没搜 `harness/facts/`——因为没有命令能列出它们。
这一步差点删掉正确的数据。
