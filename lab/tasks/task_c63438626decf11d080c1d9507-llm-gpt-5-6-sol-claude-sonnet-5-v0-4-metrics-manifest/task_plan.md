...
- 发现来源:v0.7 红队/多seed 批次的审计,记在 `lab/reports/agentic-v0.7/report.md` 与 `lab/context/research/v07-redteam-critique.md`。
- 本仓已有勘误横幅的先例:`lab/context/research/frontier-landscape-synthesis.md` 与 `lab/reports/agentic-v0.4/report.md` 顶部各有一条,照该格式写。
- 受影响面至少包括 `lab/reports/agentic-v0.4/`~`v0.7/` 下的 metrics 与 report,以及最终报告 v1.0/v2.0 的模型声明。

## Required Reading

1. `agent/llm.py`(**权威**:实际解析逻辑与返回的模型 ID)。
2. `lab/reports/agentic-v0.7/report.md`(**权威**:发现记录与影响面描述)。
3. `lab/reports/agentic-v0.4/report.md` 顶部勘误横幅(**格式范本**)。
...
仓库根或独立 worktree;允许写 `lab/reports/**` 的勘误段与 manifest、`agent/llm.py` 的标签写入处(若确有写死)。禁区:实验数值本身(只改模型标签,不动任何 fitness/指标数)、CI/oracle。
