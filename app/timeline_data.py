...
EXECUTIVE_SUMMARY = [
    ...
    },
    {
        'num': '03',
        'dimension': '真实蛋白质工程哲学',
        'dimension_en': 'Engineering Philosophy',
        'badge': '工程哲学 · 目标契约收工',
        'badge_color': '#059669',
        'badge_bg': '#ecfdf5',
        'badge_border': '#a7f3d0',
        ...
        'text': (
            '真实湿实验不存在全知上帝视角单峰假设，盲目追求单一极致极值将耗尽受限实验预算。'
...
# ---------------------------------------------------------------------------
# 四大科研认知结晶与未来蓝图 (Research Insights & Future Blueprint)
# ---------------------------------------------------------------------------
RESEARCH_INSIGHTS = [
    {
        'id': 'insight-1',
    ...
    },
    {
        ...
        'bg': '#eff6ff',
        'border': '#bfdbfe',
        'summary': '大语言模型应解耦于微观变体数值打分，而在系统外环承担假设生成、因果归因、自证伪纠偏与算法策略编排的元控制器（Meta-Controller）角色。本研究人机外环协同经 4 轮严谨对撞推导出致胜参数 β=3，确立了外环战略推演与内环数值确定性计算的分工范式。',
        'core_data': [
            {'label': '内环实验循环 (Inner Loop)', 'val': '288 预算 DBTL', 'desc': '高频数值计算层：负责代理拟合、高通量排序、确定性采集与密码学存证'},
            {'label': '外环认知循环 (Outer Loop)', 'val': 'v0.1 → v0.7 演进', 'desc': '低频元认知层：负责实证观测归因、认知自证伪、科学假设推演与策略编排'},
    ...
    {
        'id': 'insight-3',
        'title': '【核心结论 3：真实蛋白质工程决策哲学】目标驱动终止准则与高维多目标帕累托前沿寻优',
        ...
        'core_data': [
            {'label': '传统算法范式局限', 'val': '全知单峰极限假设', 'desc': '依赖完备标签先验假设，穷尽高成本实验预算以追求边际收敛极值'},
        ...
        'sections': [
            {
                'title': '🔬 产业工程范式跃迁：从理想化单点基准走向高维真实生物系统',
                'content': (
    ...
    {
        'id': 'insight-4',
        'title': '【核心结论 4：先验知识库范式演进】冷启动 71.88% 缺测盲区审计与 3D 结构空间软权重导向',
        ...
        'sections': [
            {
                ...
                'content': (
                    ...
                    '• <b>基于 3D 拓扑结构的主动引导新范式：</b>先验知识库应由被动的“空间硬门禁”演进为<b>“缺测引导雷达（Coverage-Aware Radar）”</b>——主动量化并标记残基对的认知缺损度，引导探索预算优先勘探高方差盲区；同时融入 AAV2 衣壳三维空间欧氏距离，对空间临近（<8Å）相互作用位点赋予更高权重的非线性上位表征。'
                )
            }
        ]
    }
