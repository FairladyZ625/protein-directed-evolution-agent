...
4. **LLM 归属改成实际值**(数据卫生):把 v0.4 起 metrics/manifest/报告正文里的 `gpt-5.6-sol` 改为实际解析值。**注意甄别**:`lab/agents/de-ml.json` 等 agent 声明里写 `gpt-5.6-sol` 是**正确的**(那是派工身份),不要误改;要改的只有"这一跑实际用了哪个模型"的标签。历史报告用**加勘误段**的方式更正,不覆盖原结论文字(仓内已有勘误横幅先例可照抄格式)。

## Context

- **权威审计**:`lab/context/research/assignment-coverage-audit.md`,条目 H32/H37/H39/H40/H41 与 D5 判定;证据 E4(`pdfinfo` 实测 `harness/final-report/report.pdf` 为 A4 四页、八节齐全但内容是旧 GB1 叙事)、E8(README 链接错误 + Makefile 入口错误)。
- **试题原文**:`lab/context/research/AI4S-assignment.md` 三·代码 c(README 需说明运行环境、数据来源、运行命令、主要结果)、三·独立条目(小规模数据可复现)。
- **数据卫生来源**:v0.7 红队线审计,记在 `lab/reports/agentic-v0.7/report.md`;实际解析逻辑在 `agent/llm.py`。
- **勘误格式范本**:`lab/reports/agentic-v0.4/report.md` 与 `lab/context/research/frontier-landscape-synthesis.md` 顶部各有一条勘误横幅;`lab/context/research/plateau-breaking-methods.md` 顶部是最新一条(2026-09-12)。
...
1. `lab/context/research/assignment-coverage-audit.md`(**最高权威**:H37/H39/H40/H41 与 D5)。
2. `lab/context/research/AI4S-assignment.md`(**标尺**:三·代码 c 原文)。
3. `README.md`、`Makefile`(**待改主体**)。
4. `agent/llm.py`(**权威**:实际模型解析逻辑——第 4 项必须**实跑一次拿到真实 ID**,不许凭推断改标签)。
5. `lab/context/research/v07-consolidated-summary.md`(**权威**:README 主叙事该写成什么样)。
...
- **允许写**:`README.md`、`Makefile`、`data/README.md`、smoke fixture 与其测试、`lab/reports/**` 与 `reports/**` 里的**模型标签与勘误段**、`lab/reports/pkgC-delivery/report.md`。
...
- 写 `lab/reports/pkgC-delivery/report.md`。
...
- 模型标签更正 + 历史报告勘误段。
- `lab/reports/pkgC-delivery/report.md`:改了哪些、哪些 `gpt-5.6-sol` 是正确的没改、smoke 命令与实跑输出。
