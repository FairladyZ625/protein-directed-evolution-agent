...
产出一份研究文档 `lab/context/research/plateau-breaking-methods.md`,系统梳理"主动学习/定向进化在 in-silico 代理预测遇到天花板时,人类科学家的破局策略",每条给出:①方法原理 ②代表文献/工具 ③它为什么可能突破"上位效应隐藏的峰" ④在我们的 agent 工具笼(池式、oracle=测量表查询、288 预算、工具 analyze_measured/predict/list_pool/check_knowledge/test)里能否合法落地、需要新增什么工具 ⑤**是否 answer-agnostic**(绝不能是"看着测试答案倒推")。最后给一个**按"合法性×可行性×破顶潜力"排序的候选清单**,供 CEO 裁定下一步实验。
...
1. `lab/context/research/AI4S-assignment.md` + `AI4S-domain-research.md`(试题与既有领域调研,理解任务设定)。
2. `lab/reports/agentic-v0.2/report.md` 与 `agentic-v0.3/aav/surrogate_scan.json`(平台期与诊断的定量证据)。
...
agy(Gemini)可联网检索;可写 `lab/context/research/`。若联网受限,如实说明并给出基于已有知识的最佳综述,不编造引用。
...
只读检索 + 写 `lab/context/research/plateau-breaking-methods.md`(可附一个 `plateau-breaking-shortlist.md` 或在同文档内给排序清单)。不改代码、不动实验产物、不做破坏性操作。
...
- `lab/context/research/plateau-breaking-methods.md`:方法族综述 + 五要素表 + 排序候选清单 + 3 条以内实验提名。
