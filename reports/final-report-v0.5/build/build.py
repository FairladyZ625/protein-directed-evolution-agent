...
body { font-family:"Songti SC","STSong",serif; font-size:10.1pt; line-height:1.66; }
...
figcaption {font-size:8.6pt;line-height:1.55;text-align:justify;margin-top:2.2mm;color:#506172;}
...
for i,c in enumerate(chunks,1):
 html=subprocess.run(['pandoc','--from=markdown+fenced_divs+raw_html','--to=html5','--mathml'],input=c,text=True,capture_output=True,check=True).stdout
 ...
 pages.append(f'<section class="page" data-page="{i}"><div class="running"><span>CONTROLLED DIRECTED EVOLUTION</span><span>RESEARCH REPORT · v0.5</span></div><main class="content">{html}</main><div class="footer"><span>Zeyu Li · 2026</span><span>{i} / {len(chunks)}</span></div></section>')
out='<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><title>受控科学智能体研究报告 v0.5</title><style>'+css+'</style></head><body>'+''.join(pages)+'</body></html>'
(ROOT/'scientific_report_v0.5_two_column.html').write_text(out)
