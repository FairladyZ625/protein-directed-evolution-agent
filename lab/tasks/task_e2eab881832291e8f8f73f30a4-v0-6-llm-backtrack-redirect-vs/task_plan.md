...
产出 `lab/context/research/v06-backtrack-analysis.md`,用 answer-agnostic 证据回答:
...
- **v0.4/v0.5 事实**(`lab/reports/agentic-v0.4/report.md`、`lab/reports/agentic-v0.5/report.md`,facts F-851790B5/F-C08A6CF2/F-8A58F305/F-F7BC7554):同 EpistasisRidge(CV 0.90)下,确定性 gated-greedy 288 预算直达 8.416(strong 151);LLM v0.4 停 7.829、v0.5 加自适应配比反降 6.53。v0.5 机制=rank-45 截断(4.2% 早期探索税带离纯 greedy 路径)。
...
- **前沿定位**(`lab/context/research/frontier-landscape-synthesis.md`):元层自主(判停滞+换策略)是 LLM 可能有价值的一档;微观选点 LLM 弱。

## Required Reading

1. `lab/reports/agentic-v0.5/report.md` + `lab/reports/agentic-v0.4/report.md`(**权威**:停滞曲线、rank-45、8.416 机制)。
2. `lab/reports/agentic-v0.4/aav/agentic.events.jsonl`、`agentic.metrics.json`、`deterministic.metrics.json`(v0.4 LLM vs 确定性 greedy 的实测轨迹与已测集)。
3. `lab/reports/agentic-v0.5/aav/agentic.events.jsonl`、`agentic.metrics.json`(v0.5 停滞轨迹)。
...
- **允许写范围**:仅 `lab/context/research/v06-backtrack-analysis.md`(+ 你自己在 `.worktrees/t-analysis` 内的临时分析脚本,不进产品树)。
...
- 写 `lab/context/research/v06-backtrack-analysis.md`:结论优先 + 每个数字附命令 + 对 GLM 的建议(判据 a/b/组合)。
- 台账:`ha fact record --task task_e2eab881832291e8f8f73f30a4 ...` 记承重诊断结论(≥1 fact)。

## Deliverable Contract

- `lab/context/research/v06-backtrack-analysis.md`(结论优先,≤~400 行,附实测数字与复现命令)。
