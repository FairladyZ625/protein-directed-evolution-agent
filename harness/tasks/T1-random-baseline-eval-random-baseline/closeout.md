...
- 下游复用成立:M3 的四策略对照引用了本模块的 `propose_random`,`harness/reports/workflow-v1.0/` 下的 campaign 产物可追溯到这条基线。

## Residual Risk

- 单 seed。本任务只跑了一个随机种子,因此"随机策略最弱"这个结论在本任务范围内是单点观测;真正的多 seed 稳健性要到 v0.7 的 30 seed 矩阵才建立(`harness/context/research/v07-multiseed-robustness.md`)。
