...
## 3b. 更多轮次消融:LLM 的短板不是"来不及跑",是利用效率

反驳"LLM 只是轮次不够、来不及利用 surrogate"的假设。同 seed42/门禁/上位 surrogate,每轮 48:

| 轮次 | 总预算 | cum_top10_max | 曲线要点 |
|---|---|---|---|
| 6 | 288 | 7.829 | round4 达 7.829 后平 |
| 12 | 576 | **7.829** | round5 达 7.829,**之后 7 轮全平**(strong 183) |
| 16 | 768 | **7.53** | 慢爬止于 7.53,反而更差(strong 330) |
| 确定性(6) | 288 | **8.416** | 直达真峰 |

**加到 12/16 轮(2–2.67× 预算)LLM 均未达真峰 8.416**;确定性 gated-greedy 同 surrogate 仅 288 就直达。**所以 LLM 的短板不是 budget/rounds 不足,是自主 explore/exploit 决策的利用效率确实不如纯 greedy**——给足 runway 仍到不了峰,16 轮因不同 pacing(LLM 决策变异)反而更差。这坐实结论 2:**破顶来自表征;在"利用一个好的上位 surrogate"这步,agent 的自主性是 liability 而非 asset。** 一个诚实且有价值的负结果:好的表征 + 简单贪心 > 自主 agent + 同表征。
