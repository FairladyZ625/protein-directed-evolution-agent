...
2) 甄别 gpt-5.6-sol 的两类出现:harness/agents/*.json 里的是派工身份,是正确的,不要改;要改的只有'这一跑实际用了哪个模型'的产物标签;
3) 历史报告一律加勘误段,不覆盖原结论文字。格式照抄 harness/context/research/plateau-breaking-methods.md 顶部那条(2026-09-12)。
...
收口汇报 Z:本地 commit(不 push 不开 PR)+ harness/reports/pkgC-delivery/report.md + >=1 条 fact + 两份清单(改了哪些标签 / 哪些 gpt-5.6-sol 是正确的没改)。